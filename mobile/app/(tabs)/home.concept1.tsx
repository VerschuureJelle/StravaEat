...
function TodayWeather({ weather }: { weather: Weather }) {
  ...
  const recColors = {
    good: { bg: 'rgba(46,125,50,0.15)',  text: '#4CAF50', icon: 'checkmark-circle-outline' as IoniconsName },
    ok:   { bg: 'rgba(245,127,23,0.12)', text: '#FF9800', icon: 'alert-circle-outline' as IoniconsName },
    bad:  { bg: 'rgba(198,40,40,0.15)',  text: '#EF5350', icon: 'close-circle-outline' as IoniconsName },
  }[rec.level]
        ...
        return (
          <View key={w.id} style={[pw.card, { borderLeftColor: isAI ? '#7C83FD' : color }]}>
            <View style={pw.row}>
              ...
              <Text style={[pw.sport, { color: isAI ? '#7C83FD' : '#F5F5F5' }]}>{w.sport_type}</Text>
            ...
            {w.workout_description ? (
              ...
              <View style={pw.chips}>
                ...
                {w.target_duration_min !== null && (
                  <View style={[pw.chip, { backgroundColor: '#252525' }]}>
                    <Ionicons name="time-outline" size={12} color="#8A8A8A" />
                    <Text style={[pw.chipText, { color: '#8A8A8A' }]}>{w.target_duration_min} min</Text>
                  </View>
                )}
                {w.target_hr !== null && (
                  <View style={[pw.chip, { backgroundColor: '#252525' }]}>
                    <Ionicons name="heart-outline" size={12} color="#8A8A8A" />
                    <Text style={[pw.chipText, { color: '#8A8A8A' }]}>{w.target_hr} bpm</Text>
                  </View>
                )}
              </View>
            )}
